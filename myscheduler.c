...
int execute_commands(void) {
    ...
    while (ready1 != NULL || sleeping1 != NULL || bus.process != NULL || waiting_with_no_children != NULL || num_processes_waiting_for_IO != 0) {
        if (sleeping1 != NULL && sleeping1->time <= system_time) {
        ...
        } else if (bus.process != NULL && bus.time <= system_time) {
            ...
            state_transition(bus.process, READY);
            bus.process = NULL;
        } else if (bus.process == NULL && num_processes_waiting_for_IO != 0) {
            // Commence any pendion IO if the bus is free
            move_to_bus();
        } else if (ready1 != NULL) {
            // Commence/resume the next READY process
            run_process();
        } else {
            // Jump to the next system event (waking something out of sleeping or moving off the bus)
            if (sleeping1 == NULL) {
                // If there are no sleeping processes, jump to the next process on the bus
                system_time = bus.time;
            } else if (bus.process == NULL) {
                // If there is no process on the bus, jump to the next sleeping process
                system_time = sleeping1->time;
            } else {
                // If there is a process on the bus and there are sleeping processes, jump to the next system event
