...
struct system {
    ...
    struct sleeping **bus_process; // A pointer to the sleeping process that is using the bus
...
int move_to_bus(struct system *system) {
    ...
    while (device != NULL) {
        ...
        if (device->queue_head != NULL) {
            ...
            (*system->bus_process)->process = device->queue_head; // Set the process that is using the bus to the first process in the queue of processes waiting to use the device
            ...
            (*system->bus_process)->process->next = NULL; // Set the next process of the process that is using the bus to NULL
    ...
    if (device != NULL) {
        ...
        struct syscall *syscall = (*system->bus_process)->process->syscall; // Set syscall to the syscall that the process is executing (read or write)
        ...
        printf("Process %s moving to the bus at time %d\n", (*system->bus_process)->process->command->name, *system->system_time); // Print a message to indicate that the process has moved to the bus
        (*system->bus_process)->time = *system->system_time + sleep_time; // Set the time that the process will finish using the bus
    }
    return 0; // Return 0 to indicate success
}
